...
def get_visible_runs(recording, canvas_width):
    ...
    runs = []
    for line in range(recording.code.count('\n') + 2):
...
'''
def slow_draw(canvas, recording):
    left_step = max(0, px_to_step(0) - 1)
    right_step = min(timeline['steps'], px_to_step(canvas.width) + 1)
    total_draws = 0
    for step in range(left_step, right_step):
        total_draws += 1
        if total_draws > 1000:
            break

        line = recording.line(step)
        x = step_to_px(step)
        y = line * 16   # TODO: base on editor line height
        w = 8

        if step <= timeline['mouse']:
            canvas.ink(220, 220, 200)
            if step == timeline['mouse']:
                w = 10
        else:
